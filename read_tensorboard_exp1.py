...
tempDict = {}
powerDict = {}
for subdir, dirs, files in os.walk("selected/exp1"):
    a = subdir.split("/")
...
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(6, 4.2), sharey=True)
...
ax1.grid(axis='x',linestyle='--')  # Add grid to the second subplot
ax1.set_axisbelow(True)

ax2.grid(axis='x',linestyle='--')  # Add grid to the second subplot
ax2.set_axisbelow(True)


red_patch = mpatches.Patch(color=cmap(0), label='Default governor')
fig.legend(handles=[red_patch], loc='upper right', ncol=2, bbox_to_anchor=(0.98, 1.04),frameon=False, borderpad=0.1, columnspacing = 4.0, handleheight=0.5)

r_patch = mpatches.Patch(color=cmap(4), label='zTT')
x_patch = mpatches.Patch(color=cmap(8), label='GearDVFS')
# fig.legend(handles=[r_patch, x_patch], loc='upper left', ncol=2, bbox_to_anchor=(-0.028, 1.04),frameon=False, borderpad=0.1, columnspacing = 1.0, handleheight=0.5)

# Adjust layout for the correct appearance
plt.tight_layout(pad=0.5, rect=[0, 0, 1, 0.94])  # Leave space at the top for legend
