...
class DVFStrain(Env):
    ...
    def step(self, action):
        ...
        c_states, temps, qos, t1b, t2b, littleb, midb, bigb, gpub, b, gpu_util, freqs, _, _, _ = get_states2(self.window, "fps", None, None, None)

        little_c, mid_c, big_c, gpu_c = c_states

        littleReal, midReal, bigReal, gpuReal = freqs
        ...
        info = {"little_c":little_c, "mid_c":mid_c, "big_c":big_c, "gpu_c":gpu_c, "littleReal":littleReal, "midReal":midReal, "bigReal":bigReal, "gpuReal":gpuReal, "little": little_min, "mid": mid_min, "big": big_min, "gpu": gpu_min, "qos":qos, "power":little + mid + big + gpu, "reward":reward, "ppw":ppw, "temp": [l_t, m_t, b_t, g_t, qi_t, batt_t], "util" : util_li}
