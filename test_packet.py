...
def get_packet_info(proc_num: int) -> tuple[int, int]:
    ...
    result = result.split()
    received_packet, transmitted_packet = int(result[2]), int(result[10])

    return received_packet, transmitted_packet

def monitor_packets(proc_num:str, interval:int = 0.5):
    prev_received, prev_transmitted = get_packet_info(proc_num)
    
    while True:
        time.sleep(interval)
        cur_received, cur_transmitted = get_packet_info(proc_num)
